...
namespace JadeControls.Workspace.ViewModel
{
    ...
    /*
    internal class Project : TreeNodeBase
    {
        #region Data
        ...
        public Project(Workspace workspace, TreeNodeBase parent, JadeData.Project.IProject p)
            :   base(p.Name, parent)
        {
            _workspace = workspace;
            _data = p;
            ...
            foreach (JadeData.Project.IFolder f in _data.Folders)
            {
                ProjectFolder folder = new ProjectFolder(this, this, f); 
                folder.RemoveCommand.Attach(param => this.OnRemoveFolder(param), param => this.CanDoRemoveFolder(param));
                Children.Add(folder);
            }
            ...
            foreach (JadeData.Project.IItem i in _data.Items)
            {
                if (i is JadeData.Project.File)
                {
                    File file = (new File(this, i as JadeData.Project.File));
                    file.RemoveCommand.Attach(param => this.OnRemoveFile(param), param => this.CanDoRemoveFile(param));
                    Children.Add(file);
                }
            }
        }
       ...
       //     AddFileToFolder(Children);
        ...
        public void AddFileToFolder(ProjectFolder folder)
        {
            AddProjectFileViewModel vm = new AddProjectFileViewModel();
            vm.Location = _workspace.PathDirectory;
            if (GuiUtils.DisplayModalWindow(new AddProjectFileWindow(), vm))
        ...
        public ICommand RemoveCommand
        {
            get
            {
                if (_removeCommand == null)
                {
                    _removeCommand = new RelayCommand(param => this.OnRemoveCommand(), param => this.CanDoRemoveCommand);
                }
                return _removeCommand;
            }
        }
        ...
        {
            //_parent.RemoveProject(this);
        }

        private bool CanDoRemoveCommand
        {
            get { return true; }
        }
